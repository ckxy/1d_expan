...
class HalfGanStyleMRModel(BaseModel):
    def name(self):
        return 'HalfGanStyleMRModel'

    def initialize(self, opt):
        BaseModel.initialize(self, opt)
        self.isTrain = opt.isTrain
        # define tensors
        self.input_A = self.Tensor(opt.batchSize, opt.input_nc + opt.n_pic,
        ...
        if self.isTrain:
            ...
            self.netD = networks.define_D(opt.output_nc, opt.ndf,
                                          ...
                                          opt.n_layers_D, opt.norm, use_sigmoid, self.gpu_ids, opt.n_pic)
    ...
    def forward(self):
        ...
        self.mask = Variable(self.input_M)

    def ttt(self):
        a = Variable(torch.rand(1, 3, 128, 128)).cuda()
        b = self.netG(a)
        print(b.size())
        # c = self.netD(b)
        # print(c.size())
    ...
    def backward_G(self):
        ...
        # Second, G(A) = B
        # print(self.input_M.size(), self.input_B.size(), self.real_B.size())
        if self.opt.use_mask:
            self.loss_G_L1 = self.criterionL1(self.fake_B * self.mask, 
                self.real_B * self.mask) * self.opt.lambda_A
        else:
            # print(self.fake_B.size(), self.real_B.size())
    ...
    def get_current_visuals(self):
        real_A = util.tensor2im(self.real_A.data[:1,:,:,:])
        fake_B = util.tensor2im(self.fake_B.data[:1,:,:,:])
        real_B = util.tensor2im(self.real_B.data[:1,:,:,:])
        return OrderedDict([('real_A', real_A), ('fake_B', fake_B), ('real_B', real_B)]), [(0,0)]
