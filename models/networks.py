...
def define_G(input_nc, output_nc, ngf, which_model_netG, norm='batch', use_dropout=False, gpu_ids=[], padding_type='reflect'):
    ...
    if which_model_netG == 'resnet_4blocks':  # +128
        netG = ResnetGenerator(input_nc, output_nc, ngf, norm_layer=norm_layer, use_dropout=use_dropout, n_blocks=4,
                               gpu_ids=gpu_ids)
    elif which_model_netG == 'resnet_1blocks':  # +32
        netG = ResnetGenerator(input_nc, output_nc, ngf, norm_layer=norm_layer, use_dropout=use_dropout, n_blocks=1,
                               gpu_ids=gpu_ids)
    elif which_model_netG == 'resnet_2blocks':  # +64
        netG = ResnetGenerator(input_nc, output_nc, ngf, norm_layer=norm_layer, use_dropout=use_dropout, n_blocks=2,
                               gpu_ids=gpu_ids)
    elif which_model_netG == 'resnet_8blocks':  # +256
        netG = ResnetGenerator(input_nc, output_nc, ngf, norm_layer=norm_layer, use_dropout=use_dropout, n_blocks=8,
                               gpu_ids=gpu_ids)
    else:
...
def define_D(input_nc, ndf, which_model_netD,
             n_layers_D=3, norm='batch', use_sigmoid=False, gpu_ids=[], n_pic=1):
    netD = None
    use_gpu = len(gpu_ids) > 0
    norm_layer = get_norm_layer(norm_type=norm)
...
class ResnetGenerator(nn.Module):
    def __init__(self, input_nc, output_nc, ngf=64, norm_layer=nn.BatchNorm2d, use_dropout=False, n_blocks=6,
        ...
        #     model += [ResnetBlock1(ngf * mult, padding_type=padding_type, norm_layer=norm_layer, use_dropout=use_dropout,
        #                           use_bias=use_bias)]
        for i in range(n_blocks):
            model += [DeconvResnetBlock(ngf * mult, padding_type=padding_type, norm_layer=norm_layer, use_dropout=use_dropout,
                                  use_bias=use_bias)]
